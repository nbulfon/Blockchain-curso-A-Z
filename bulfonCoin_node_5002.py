...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_adress, receiver = 'Valentina Bulfón', amount = 10)
...
# endpoint para dar de alta un nuevo nodo
@app.route('/connect_node', methods = ['POST'])
def connect_node():
    ficheroJson = request.get_json()
    nodes = ficheroJson.get('nodes')
    
    # si no hay nodos para añadir devuelvo msj y corto la ejecución
    if nodes is None:
    ...
    response = {
        'message': 'Todos  los nodos han sido conectados. La cadena de bulfonCoin ahora contiene los siguientes nodos:',
        'total_nodes': list(blockchain.nodes)
        }
    return jsonify(response),201

#endpoint para la función del protocolo de concenso.
# o sea, reemplazar la cadena por la más larga (si es necesario).
